...
def visualizeParams(x, img_cache ,mask_cache):
    ...
    for idx in range(img_num - 5, img_num):
        ...
        prevStack = vStack
    cv2.imshow("Adjusting params", vStack)
...
def substract_background(dir_info, kernelSize_medianBlur, kernelSize_dilation, areaThreshold):
    ...
    if key == 32:
        for idx in range(len(img_cache)):
            ...
            save_result(mask_cache[idx], filename, binary_mask_dir)
            writeLabel((x1,x2,y1,y2), annotation_dir, filename)
    cap.release()
    cv2.destroyAllWindows()

def main(working_dir, folder_idx, h_resize, w_resize,  kernelSize_medianBlur, kernelSize_dilation, areaThreshold):

    img_folder = "{}/vid{}".format(working_dir,folder_idx)
    label_dir = "{}/vid{}_mask".format(working_dir, folder_idx)
...
if __name__ == '__main__':
    ...
    kernelSize_dilation = 21
    h_resize = 1480
    w_resize = 2048
    cv2.namedWindow("Adjusting params")
    main(working_dir,folder_idx, h_resize, w_resize, kernelSize_medianBlur, kernelSize_dilation, areaThreshold)
